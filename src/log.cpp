...
namespace mos
{
  ...
  log_msg(const char * format, ...)
  {
    ...
    vfprintf(MOS_DATA->logfile, format, ap);
  }
  ...
  // fuse context
  void
  log_fuse_context(struct fuse_context * context)
  {
    log_msg("    context:\n");
    ...
    log_struct(((struct mos_state *)context->private_data), logfile, % 08x, );
    log_struct(((struct mos_state *)context->private_data), rootdir, % s, );
  ...
  void
  log_retstat(char * func, int retstat)
  {
    int errsave = errno;
  ...
  int
  log_syscall(char * func, int retstat, int min_ret)
  {
